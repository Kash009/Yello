...
.factory('Socket', function (socketFactory) {
  var myIoSocket = io.connect('https://yello.azurewebsites.net');
...
.controller('LoginController', function($scope, $state, $cordovaOauth, $http){
  ...
  $scope.loginWithFacebook = function(){
    $cordovaOauth.facebook("1747391172141766", ["email"]).then(function(result) {
            //alert(result.access_token);
            $http.get('https://graph.facebook.com/v2.6/me?fields=id,name,picture&access_token=' + result.access_token)
...
.controller('ChatController', function($scope, $state, $timeout, $stateParams, Socket, $ionicScrollDelegate, $sce, $cordovaMedia){

    $scope.status_message = "Welcome to Yello";
    $scope.messages = [];
    $scope.nickname = $stateParams.data.nickname;
    $scope.displayPicture = $stateParams.data.displayPicture;

    var COLORS = ['#f44336', '#E91E63', '#9C27B0', '#673AB7', '#3F51B5', '#009688', '#808080', '#663399'];
